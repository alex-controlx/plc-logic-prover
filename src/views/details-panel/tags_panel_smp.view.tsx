...
const logger = new Logger("SmpTagsPanelView", true);

const noTagsMessage = "No tags to show.";
const sortDirections: number[] = [0, 0, 0];
...
interface SmpTagsPanelViewProps {}

interface SmpTagsPanelViewState {
    searchValue: string,
...
export default class SmpTagsPanelView extends Component<SmpTagsPanelViewProps, SmpTagsPanelViewState> {
    ...
    private columns: string[] = ["Address", "Data Type", "Description", "Used"];
    private cellChangeBuffer: string | null = null;
    private tableElement: Element | null = null;


    constructor(props: SmpTagsPanelViewProps) {
        super(props);

        logger.log("constructor()");

        const columnWidths = this.tagPageState.columnWidthsSmp;
    ...
    //                 tagname: tagFields[this.clipboardTagIndex],
    //                 program: "",
    //                 datatype,
    //                 desc: tagFields[this.clipboardDescIndex],
    //             }
    //             results.push(tag);
    //         }
    //     }
    ...
    private onColumnWidthChanged = (index: number, size: number) => {
        ...
        this.tagPageState.columnWidthsSmp = [...columnWidths];
    ...
    private getCellData = (rowIndex: number, columnIndex: number): [string, number] => {
        ...
        switch (columnIndex) {
            case 0: return [abTag.smpAddress || '', rowIndex];
            case 1: return [abTag.datatype, rowIndex];
            case 2: return [abTag.desc, rowIndex];
            case 3: return [abTag.id, rowIndex];
        }
    ...
    private sortColumn = (callback?: () => void) => {
        ...
        if (this.state.searchValue) {
            ...
            sortedIndexMap = sortedIndexMap
                .filter(item => (
                    (this.tagsUI[item].smpAddress || '').toLowerCase().includes(normalizedSearchValue) ||
        ...
        if (sortDirections.findIndex(item => item !== 0) >= 0) {
            ...
            sortedIndexMap.sort((a: number, b: number) => {
                switch (columnIndex) {
                    case 0: return comparator((this.tagsUI[a].smpAddress || ''), this.tagsUI[b].smpAddress || '');
                    case 1: return comparator(this.tagsUI[a].datatype, this.tagsUI[b].datatype);
                    case 2: return comparator(this.tagsUI[a].desc, this.tagsUI[b].desc);
                    case 3: return tagUsageSorting(isAcs, this.tagsUI[a].usage, this.tagsUI[b].usage);
                }
                return 0;
            });
        }
                                ...
                                (
                                    <Popover popoverClassName="used-more--list" position={PopoverPosition.AUTO}>
                                        <Button
                                            ...
                                            minimal={true}
                                        >
                                            <Icon className="used-more--icon" icon={IconNames.MORE} iconSize={14}/>
                                        </Button>
                                        <Menu>
                                            {
    ...
    private editCell = async (event: CellEditEvent, value: string, row?: number, col?: number) => {
        ...
        if (event === CellEditEvent.onChange) {
            ...
            if (col === 0) {
                if (this.cellChangeBuffer === null) this.cellChangeBuffer = abTag.smpAddress || '';
                abTag.smpAddress = value;
            } else if (col === 2) {
        ...
        if (event === CellEditEvent.onConfirm) {
            const isNoChanges = (
                (col === 0 && abTag.smpAddress === this.cellChangeBuffer) ||
                (col === 2 && abTag.desc === this.cellChangeBuffer) ||
